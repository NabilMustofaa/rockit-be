...
const joinRoom = async (req, res) => {
  ...
  try {
    ...
    const gameCheck = await pool.query("SELECT * FROM games WHERE token = $1", [
      token,
    ]);
    ...
    const player_1_username = userQuery.rows.find(
      (user) => user.id === game.player_1_id
    ).username;
    const player_2_username = userQuery.rows.find(
      (user) => user.id === game.player_2_id
    ).username;
...
const startGame = async (req, res) => {
  ...
  try {
    const gameQuery = await pool.query("SELECT * FROM games WHERE token = $1", [
      token,
    ]);
  ...
  try {
    ...
    const combinedRounds = rounds.rows.reduce((acc, curr) => {
      ...
      if (existingRound.player_1_move && existingRound.player_2_move) {
        existingRound.result = getRoundWinner(
          existingRound.player_1_move,
          existingRound.player_2_move
        );
      }
    ...
    const gameResults = combinedRounds.reduce(
      (count, round) => {
    ...
    const winner =
      gameResults.player1Wins > gameResults.player2Wins
        ? "Player 1"
        : gameResults.player2Wins > gameResults.player1Wins
        ? "Player 2"
        : "Draw";
